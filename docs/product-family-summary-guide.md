...
<# <Conditional Test="Row_Type='Subtotal' or Row_Type='Section Total' or Row_Type='Grand Total'"> #>
<# <Value Select="Display_Label"/> #>     <# <Value Select="Amount_Net"/> #>
<# </Conditional> #>
<# </Repeating> #>
```

Style by `Row_Type__c`: bold + top border for `Section Total`, double border + larger font for `Grand Total`, normal for `Subtotal`, using the Word formatting around a further nested `Conditional Test="Row_Type='Grand Total'"` — the tag controls whether a row prints, Word formatting controls how it looks.
